...
class NPCChatResponse(BaseModel):
    """NPC 對話回應 (用於與 NPC 聊天時，包含相關性評分)"""
    npc_reply: str = Field(description="NPC 的角色扮演回覆 (English)")
    feedback_chi: str = Field(description="語法/用詞建議 (繁體中文)，若無問題則為空字串", default="")
    feedback_eng: str = Field(description="Grammar/vocabulary feedback (English)，若無問題則為空字串", default="")
    is_english: bool = Field(description="使用者是否使用英文", default=True)
    relevance_score: int = Field(description="問題相關性評分 (1-10)，與主題相關且問到重點才給高分", ge=1, le=10, default=5)
    language_score: int = Field(description="語言品質評分 (1-10)，文法與用詞正確性", ge=1, le=10, default=5)
...
class GameInteractionLog(BaseModel):
    ...
    interaction_type: str = Field(description="互動類型: npc_chat 或 question_answer")
    theme_id: str = Field(description="主題ID")
    level_idx: int = Field(description="關卡索引", default=-1)
    question_idx: int = Field(description="題目索引", default=-1)
    npc_idx: int = Field(description="NPC索引", default=-1)
    npc_name: str = Field(description="NPC名稱", default="")
    user_transcript: str = Field(description="使用者輸入的文字")
    ai_response: str = Field(description="AI的回應")
    relevance_score: int = Field(description="相關性分數 (NPC聊天用)", default=0)
    language_score: int = Field(description="語言品質分數", default=0)
    total_score: int = Field(description="總分 (問題回答用)", default=0)
    is_correct: bool = Field(description="答案是否正確 (問題回答用)", default=False)
    feedback_chi: str = Field(description="中文回饋", default="")
    feedback_eng: str = Field(description="英文回饋", default="")
